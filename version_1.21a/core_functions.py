...
def ExcelParse(excelPath, excelSheet, outputPath, hyperionColumns, workOrderColumns):
    ...
    dfIndicies = df.columns.values
    hyperionInterp = []
    hyperionMatcher = []

    for index in dfIndicies:
        hyperionInterp.append(index)

    for i in hyperionColumns:
        for b in hyperionInterp:
            if i in b:
                hyperionMatcher.append(b)                      
            

    #Reorders Columns to Match Previous Hyperion Form
    dfHyperion = pd.DataFrame(df, columns = hyperionMatcher)
